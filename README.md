...
The project is `@dehubapp/dehubtv`. Builds authenticate with `EXPO_TOKEN`; the
Android keystore for `io.dehub.tv` was generated by EAS and is separate from the
phone app's certificates, which must not be touched.

**A cloud build does not see `.env`.** EAS respects `.gitignore` when it packs
the project, so every variable comes from the fallbacks in `config/env.ts` — all
of which are the real production values for exactly that reason. If a genuine
secret is ever needed, it goes in `eas env:create`, never in a fallback.

### Getting it onto a television

```bash
adb connect <tv-ip>:5555
adb install -r dehub-tv.apk
```

The TV's IP is under Settings → Network, and ADB debugging has to be switched on
first (Settings → Device preferences → Developer options). Without a computer,
the usual route is the Downloader app from the Play Store, pointed at the build
URL.

It appears on the launcher's apps row as **DeHub TV** with the banner from
`assets/tv-banner.png` — not in the phone-style app list, because the manifest
carries the leanback launcher category.
